...
- Для получения 🆔 `TG_CHAT_ID` воспользуйтесь @userinfobot (ботом в телеграме).
  > Необходим для получения ошибок от ботов.
- 🔑 **токен ВКонтакте:**
  Для получения токена нужно создать [группу](https://vk.com/groups?w=groups_create_new) в ВКонтакте  
  ⚙️ Включите в настройках группы ВК  
  **Включить сообщения:** Управление сообществом → Сообщения → Сообщения сообщества: Включены.  
  <img style="max-width:100%; height:auto;" alt="Снимок экрана от 2026-07-16 13-09-35" src="https://github.com/user-attachments/assets/ce6808e8-a58c-44b7-aa2b-5190bb6fea08" />
...
<img style="max-width:100%; height:auto;" alt="Снимок экрана от 2026-07-16 13-07-16" src="https://github.com/user-attachments/assets/e0903a0a-c400-46d8-b3df-4f3f9418a6c2" />

**Выбрать типы событий:** Там же, на вкладке «Типы событий», обязательно поставьте галочку напротив «Входящие сообщения» (message_new).
...
<img style="max-width:100%; height:auto;" alt="Снимок экрана от 2026-07-16 12-54-42" src="https://github.com/user-attachments/assets/763df98c-45aa-4574-8a4c-c6529af8956a" />
...
```
TG*BOT_TOKEN=ваш*токен*телеграм*бота
VK*BOT_TOKEN=ваш*токен_вконтакте
...
```
```
Напишите ответ текстом — бот сравнит с правильным.
```
```
...
vk:user:<id>:current_answer / current_answer (для VK).
```

Проверка ответа — rapidfuzz.token_sort_ratio:

```
≥ 80 → correct # Верно. Правильный ответ.

≥ 50 → close # Близко. Похоже, но не точно.

иначе → wrong # Не верно. Правильный ответ.
```

Вопрос выбирается случайно из общего списка (random.choice).

⚖️ Лицензия
MIT License

🎡 Цель проекта
Код написан в образовательных целях на онлайн-курсе для разработчиков dvmn.org.
